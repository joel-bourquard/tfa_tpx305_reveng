...
int main()
{
  ...
  while (1) {
	...
	/*if (nbrLines == 4)
	{
		printf("%d lines is enough for now. Exiting...\n", nbrLines);
		break;
	}*/
  }
  fclose(fd);
  ...
  int oldPercentage = 0;
  //seed1 = 0xa819; {
  for (seed1=0; seed1<(1<<width); seed1++) {
	float percentage = (100*seed1)/(float)(1<<width);
	if (percentage == 0 || percentage >= oldPercentage+5) {
	  printf("Progress: %f percent\n", percentage);
	  oldPercentage = percentage;
	}
    for (seed2=0; seed2<(1<<width); seed2++) {
      int ignoredDataLength; // 0xff + 2-byte checksum

	  int idxDataStart;
	  for (ignoredDataLength=2; ignoredDataLength<=3; ignoredDataLength++)
	  for (idxDataStart=0; idxDataStart<nbrBytes-ignoredDataLength; idxDataStart++) {
		int dataLength = (nbrBytes-ignoredDataLength) - idxDataStart;
	    int idxLine;
		if (dataLength > 0) {
	    int success = 1;
        for (idxLine=0; idxLine<nbrLines; idxLine++) {
          uint8_t *theBytes = &allLines[idxLine][0];
          uint8_t *theData = &allLines[idxLine][idxDataStart];

          //printf("\n");
		  uint16_t result;
		  if (width == 8) {
	        result = lfsr_digest8(theData, dataLength, seed1, seed2);
            uint8_t result0 = result & 0xff;
	        int matchA = (result0 == theBytes[idxChecksum] );
	        if (!matchA) {
              success = 0;
			  break;
            }
	        ...
	        result = /*crc16*/ lfsr_digest16(theData, dataLength, seed1, seed2);
            uint8_t result1 = (result >> 8) & 0xff;
            uint8_t result0 = result & 0xff;
	        int matchA = ( result0 == theBytes[idxChecksum] ) && ( result1 == theBytes[idxChecksum+1] );
	        int matchB = ( result1 == theBytes[idxChecksum] ) && ( result0 == theBytes[idxChecksum+1] );
	        if (!(matchA || matchB)) {
              success = 0;
			  break;
            }
	      ...
	      //printf("reference checksum = %02x%02x\n", theBytes[idxChecksum], theBytes[idxChecksum+1]);
	      //printf("calculated checksum = %04x\n", result);
        }
		if (success) {
          printf("Data length: %d, all samples match! (seed1 = %x seed2 = %x)\n", dataLength, seed1, seed2);
		}
		}
      }
	}
  }
  printf("Search is fully finished.\n");
}
